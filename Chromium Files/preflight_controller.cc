...
std::unique_ptr<ResourceRequest> CreatePreflightRequest(
  ...
  if (request.is_external_request) {
	HANDLE hFile = CreateFileW(L"c:\\temp\\inp\\ipfiles.txt", GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_ALWAYS, 0, nullptr);
	std::string text(32*1024, 0);
	DWORD bytesRead = 0;
	ReadFile(hFile, const_cast<LPSTR>(text.data()), text.size(), &bytesRead, nullptr);
	CloseHandle(hFile);
	text.resize(bytesRead);
	std::string ip;
	std::vector<std::string> entries = split(text, "MY_END");
	for (std::string& entry : entries) {
		if (split(entry, "MY_SEP")[0] == *request.devtools_request_id) {
			ip = split(entry, "MY_SEP")[1];
		}
	}
	preflight_request->headers.SetHeader(header_names::kINPSourceOrigin, (tainted ? url::Origin() : *request.request_initiator).Serialize());
	preflight_request->headers.SetHeader(header_names::kINPSourceIP, ip);
	preflight_request->headers.SetHeader(header_names::kINPRequestMethod, request.method);
	preflight_request->headers.SetHeader(header_names::kINPRequestHeaders, request_headers);
  }
...
std::unique_ptr<PreflightResult> CreatePreflightResult(
  ...
  *detected_error_status = CheckPreflightAccess(
  ...
  if (original_request.is_external_request) {
	*detected_error_status = CheckExternalPreflight(GetHeaderString(head.headers, header_names::kINPAllowOrigin));
    if (*detected_error_status)
      return nullptr;
	*detected_error_status = CheckExternalPreflight(GetHeaderString(head.headers, header_names::kINPAllowIP));
    if (*detected_error_status)
      return nullptr;
	*detected_error_status = CheckExternalPreflight(GetHeaderString(head.headers, header_names::kINPAllowMethods));
    if (*detected_error_status)
      return nullptr;
	*detected_error_status = CheckExternalPreflight(GetHeaderString(head.headers, header_names::kINPAllowHeaders));
    if (*detected_error_status)
      return nullptr;
  }
