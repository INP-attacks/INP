...
base::Optional<ResourceRequestBlockedReason> ResourceFetcher::PrepareRequest(
  ...
  builder.Append('.');
  builder.AppendNumber(rand());
  auto id =  builder.ToString();
  resource_request.SetDevToolsId(id);
  ...
  auto text = resource_request.GetDevToolsId();
  ...
  auto text3 = properties_->GetFetchClientSettingsObject().GetIPAddress();
  dwBytesWritten = 0;
