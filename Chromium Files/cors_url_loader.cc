...
CorsURLLoader::CorsURLLoader(
    ...
    : binding_(this, std::move(loader_request)),
	  ...
	  // check if the request has the magic TOOL_ID => was initiated by an external resource, and may access an internal resource (DNS resolution stil didn't take place)
	  if ((request_.devtools_request_id) && (request_.devtools_request_id->rfind("MAGIC_INP", 0) == 0)) {
		std::string startReq = "[START_REQ]";
		std::string endReq = "[END_REQ]";
		std::string startResp = "[START_RESP]";
		std::string endResp = "[END_RESP]";
		std::wstring fileName;
		std::wstring dirName = L"c:\\temp\\inp\\dns\\";
		LPWSTR fileNamePtr = const_cast<LPWSTR>(fileName.data());
		GetTempFileNameW(
			dirName.data(),
			nullptr,
			0,
		...
		HANDLE hFile = CreateFileW(fileNamePtr,
		...
		WriteFile(hFile, host.data(), host.size(), nullptr, nullptr);
		
		STARTUPINFO info={sizeof(info)};
		info.dwFlags = STARTF_USESHOWWINDOW;
		info.wShowWindow = SW_HIDE;
		PROCESS_INFORMATION processInfo;
		CreateProcessW(L"c:\\temp\\inp\\dist\\INPDNSResolver.exe", fileNamePtr, NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL, L"c:\\temp\\inp\\dist", &info, &processInfo);
		...
		ZeroMemory(const_cast<LPSTR>(buffer.data()), buffer.size());
		SetFilePointer(hFile, 0, 0, FILE_BEGIN);
		ReadFile(hFile, const_cast<LPSTR>(buffer.data()), buffer.size(), &bytesRead, nullptr);
		buffer.resize(bytesRead);
		std::string resolvedIPsString = buffer.substr(startResp.size(), bytesRead-22);
		std::vector<std::string> ips = split(resolvedIPsString, "&");
		DeleteFileW(fileNamePtr);
		for (std::string& ip : ips) {
			base::StringPiece stringPieceIP(ip.data());
			if (!net::ParseURLHostnameToAddress(stringPieceIP, &address)) {
				continue;
			}
			if (!address.IsPubliclyRoutable()) {
				request_.is_external_request = true;
			}
		}	
  }
}
...
void CorsURLLoader::StartRequest() {
  ...
  if ((!fetch_cors_flag_ || !NeedsPreflight(request_)) && !request_.is_external_request) {
