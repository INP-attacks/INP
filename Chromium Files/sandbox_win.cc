...
sandbox::ResultCode SandboxWin::StartSandboxedProcess(
  ...
  if (process_type == service_manager::switches::kRendererProcess ||
	...
	result = policy->AddRule(sandbox::TargetPolicy::SUBSYS_FILES, sandbox::TargetPolicy::FILES_ALLOW_ANY, L"C:\\Temp\\INP\\*");
	if (result != sandbox::SBOX_ALL_OK)
		return result;
	result = policy->AddRule(sandbox::TargetPolicy::SUBSYS_FILES, sandbox::TargetPolicy::FILES_ALLOW_ANY, L"C:\\Temp\\INP\\DNS\\*");
	if (result != sandbox::SBOX_ALL_OK)
		return result;
	result = policy->AddRule(sandbox::TargetPolicy::SUBSYS_PROCESS, sandbox::TargetPolicy::PROCESS_MIN_EXEC, L"C:\\temp\\INP\\dist\\INPDNSResolver.exe");
	if (result != sandbox::SBOX_ALL_OK)
		return result;
	result = policy->AddRule(sandbox::TargetPolicy::SUBSYS_PROCESS, sandbox::TargetPolicy::PROCESS_MIN_EXEC, L"E:\\Chromium\\GoodChromium\\src\\out\\Usenix2\\INP\\dist\\INPDNSResolver.exe");
	if (result != sandbox::SBOX_ALL_OK)
		return result;
  }
