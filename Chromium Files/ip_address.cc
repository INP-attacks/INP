...
bool IsPubliclyRoutableIPv4(const IPAddressBytes& ip_address) {
	...
	uint8_t currentAddress[4];
	size_t currentPrefix;

	HANDLE hFile = CreateFileW(L"C:\\Temp\\inp\\ipAddresses.txt", GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING, 0, nullptr);
	char resultBuf[1024];
	DWORD bytesRead = 0;
	ReadFile(hFile, resultBuf, 1024, &bytesRead, nullptr);
	CloseHandle(hFile);
	std::string result(resultBuf, bytesRead);	
	
	auto addresses = split(result, "&");
	
	for (size_t i = 0; i < addresses.size(); ++i) {
		auto aa = addresses[i];
		//print_adapter(aa);
		auto address_parts = split(aa.substr(0, aa.find("//")), ".");
		currentAddress[0] = std::atoi(address_parts[0].c_str());
		currentAddress[1] = std::atoi(address_parts[1].c_str());
		currentAddress[2] = std::atoi(address_parts[2].c_str());
		currentAddress[3] = std::atoi(address_parts[3].c_str());
		auto currentPrefixString = split(result, "/")[1];
		currentPrefix = std::atoi(currentPrefixString.c_str());
		
		if (IPAddressPrefixCheck(ip_address, (const uint8_t*)currentAddress, currentPrefix)) {
			return false;
		}
	}	

	// check if IP address is local (127.0.0.0/8)
	currentAddress[0]=127;
	currentAddress[1]=0;
	currentAddress[2]=0;
	currentAddress[3]=0;
	currentPrefix = 8;
